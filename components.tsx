...
export function Header({ isInstallable, onInstallClick, onSettingsClick }: { isInstallable: boolean; onInstallClick: () => void; onSettingsClick: () => void; }): HTMLElement {
...
export function Navigation(): HTMLElement {
  ...
  const createNavButton = (view: View, label: string, icon: string) => {
  ...
  nav.querySelectorAll('.nav-button').forEach(button => {
    button.addEventListener('click', () => {
      const view = button.getAttribute('data-view') as View;
      if (view) {
